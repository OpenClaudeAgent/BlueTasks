...
export function SettingsDialog({
  ...
  function requestCategoryDelete(id: string, name: string) {
    if (busy) {
      return;
    }
    const taskCount = taskCountByCategoryId[id] ?? 0;
    setPendingCategoryDelete({id, name, taskCount});
  }
  ...
  async function confirmCategoryDelete() {
    ...
    setBusy(true);
    try {
      await categoriesApi.remove(pending.id);
      await onCategoriesUpdated();
    } finally {
      setBusy(false);
    }
  ...
  const deleteDescription =
    pendingCategoryDelete != null
      ? pendingCategoryDelete.taskCount > 0
        ? t('settingsDeleteCategoryWithTasks', {
            name: pendingCategoryDelete.name,
            count: pendingCategoryDelete.taskCount,
          })
        : t('settingsDeleteCategoryConfirm', {name: pendingCategoryDelete.name})
      : '';
    ...
    <>
                              ...
                              {editingId === cat.id ? (
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    disabled={busy}
                                    onClick={() => requestCategoryDelete(cat.id, cat.name)}
                                    type="button"
                                  >
                                    <Trash2 size={15} />
                                  </button>
      ...
      >
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content className="confirmDialog__content">
            <AlertDialog.Title className="confirmDialog__title">
              {t('settingsDeleteCategoryDialogTitle')}
            </AlertDialog.Title>
