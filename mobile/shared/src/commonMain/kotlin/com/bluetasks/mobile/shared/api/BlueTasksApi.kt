...
import com.bluetasks.mobile.shared.http.BlueTasksJson
...
public class BlueTasksApi(
        ...
        runCatching {
            val response: HttpResponse =
                client.post("/api/tasks") {
                    ...
                    setBody(BlueTasksJson.encodeToString(TaskWritePayload.serializer(), payload))
                }
            if (!response.status.isSuccess()) {
                throw ApiException(response.status.value, response.bodyAsText())
            }
        ...
        runCatching {
            val response: HttpResponse =
                client.post("/api/categories") {
                    ...
                    setBody(BlueTasksJson.encodeToString(CreateCategoryBody.serializer(), body))
                }
            if (!response.status.isSuccess()) {
                throw ApiException(response.status.value, response.bodyAsText())
            }
    ...
    public suspend fun updateCategory(
        ...
        runCatching {
            val response: HttpResponse =
                client.put("/api/categories/$id") {
                    headers {
                        append(HttpHeaders.ContentType, ContentType.Application.Json.toString())
                    }
                    setBody(BlueTasksJson.encodeToString(UpdateCategoryBody.serializer(), body))
                }
            if (!response.status.isSuccess()) {
                throw ApiException(response.status.value, response.bodyAsText())
            }
