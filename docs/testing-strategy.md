...
- **Before adding a test**, name the behaviour: e.g. “quick capture on Today assigns today’s date”, “export DB triggers download with server filename”, “invalid category filter resets to All”.
...
- **Server validation**: [`taskSanitize.ts`](../server/src/taskSanitize.test.ts), [`categoryIconIds.ts`](../server/src/categoryIconIds.test.ts).
