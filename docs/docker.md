...
- **Dockerfile**: **`deps`** runs `npm ci --omit=dev -w @bluetasks/server` on Linux, then **`docker-prune-native-deps.mjs`** copies only **`better-sqlite3` and its runtime dependencies** (excluding install-only tooling such as `prebuild-install`). **`runtime`** copies that subset plus **`server/dist/docker-bundle.cjs`** (esbuild bundle: Express, CORS, Multer, etc.), `web/app/dist`, and `server/data/area-icon-ids.json`. No `package.json` in the final image. Entrypoint: `node server/dist/docker-bundle.cjs`.
